...
import { Schemas, emailRegex, passwordRegex, userNameRegex } from '/imports/api/Schemas';
...
Meteor.methods({
  async validateNewUser(userOptions) {
    ...
    if (!emailRegex.test(userOptions.email)){
      throw new Meteor.Error('invalid-email', "The input email format is invalid!");
    };
    ...
    if (existingEmail) {
      throw new Meteor.Error('already-in-use-email', "This email has already been used!");
    }

    //Validate the password:
    if (!passwordRegex.test(userOptions.password)){
      throw new Meteor.Error('invalid-password', `Password is invalid:
