...
export const EventLeaderboardList = ({ eventId }) => {
    ...
    const proofs = useFind(
        ProofCollection,
        [{eventId: {$eq: eventId}},{fields: {username: 1, user: 1, upvotes: 1}, sort: {upvotes: -1}}],
        [eventId]
    );
    ...
    const leaderboard = proofs?.map(proof => ({...proof, ...profiles?.find(profile => profile._id === proof.user)}));
    ...
    return(
        <div>
            <div
                className='grid grid-cols-3 gap-1 w-full justify-end-safe items-end'
            >
