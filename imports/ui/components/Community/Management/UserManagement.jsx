...
import { FiSearch, FiEye, FiEdit3, FiMail, FiTrash2 } from 'react-icons/fi';
...
export const UserManagement = () => {
  ...
  // Fetch User Ids -> then retrieve user id instances
  const fetchSkillTreeUsers = async () => {
    try {
      // Get SkillTree
      const skillTree = await Meteor.callAsync('skilltrees.get', skilltreeID);

      // Get all members of the skilltree
      const allSkillTreeMembersID = skillTree?.subscribers ?? [];

      // Get all skilltree member instances
      const allSkillTreeMembers = await Meteor.callAsync(
        'users.getMultipleByIds',
        allSkillTreeMembersID
      );

      setUsers(allSkillTreeMembers);
    } catch (error) {
          ...
          <tbody>
            {filteredUsers.map(user => (
              ...
              >
                ...
                <td className="py-4 px-4">
                  <span
                    className={`px-3 py-1 rounded-full text-sm font-medium border ${getRoleColor(user.roles)}`}
                  >
                    {getRoleDisplay(user)}
                  </span>
                </td>
                <td className="py-4 px-4">
                  <span
                    className={`px-3 py-1 rounded-full text-sm font-medium border ${getStatusColor(user.isActive)}`}
                  >
